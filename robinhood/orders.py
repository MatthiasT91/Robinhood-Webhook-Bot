...
@login_required
def order_buy_crypto_by_quantity(symbol, _id, quantity, timeInForce='gtc', jsonify=True):
    ...
    """ 
    return order_crypto(symbol, _id, "buy", quantity, "quantity", None, timeInForce, jsonify)


@login_required
def order_buy_crypto_limit(symbol, quantity, limitPrice, _id, timeInForce='gtc',jsonify=True):
    ...
    """ 
    return order_crypto(symbol, _id, "buy", quantity, "quantity", limitPrice, timeInForce, jsonify)
...
@login_required
def order_sell_crypto_by_quantity(symbol, _id, quantity, timeInForce='gtc', jsonify=True):
    """Submits a market order for a crypto by specifying the decimal amount of shares to buy.
    Good for share fractions up to 8 decimal places.
    ...
    """ 
    return order_crypto(symbol, _id, "sell", quantity, "quantity", None, timeInForce, jsonify)


@login_required
def order_sell_crypto_limit(symbol, quantity, limitPrice,  _id, timeInForce='gtc', jsonify=True):
    ...
    """
    return order_crypto(symbol, _id, "sell", quantity, "quantity", limitPrice, timeInForce, jsonify)
...
@login_required
def order_crypto(symbol, _id, side, quantityOrPrice, amountIn="quantity", limitPrice=None, timeInForce="gtc", jsonify=True):
    ...
    crypto_id = _id #get_crypto_id(symbol)
    orderType = "market"
    ...
    if limitPrice:
        price = limitPrice #round_price(limitPrice)
        orderType = "limit"
    else:
    ...
    if amountIn == "quantity":
        ...
        quantity = quantityOrPrice/price #round_price(quantityOrPrice/price)
